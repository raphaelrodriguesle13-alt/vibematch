...
Se a rotação falhar, for inválida/reutilizada, incoerente ou ocorrer durante logout/troca de conta, o cliente limpa as credenciais locais e aciona logout fail-closed. Quando há refresh válido, o logout chama `POST /auth/logout/refresh` com `{ "refresh_token": "..." }`, sem `Authorization`, e aceita somente `200 {"ok":true}` como confirmação; `503 REVOCATION_UNAVAILABLE` permanece visível como revogação não confirmada. O endpoint server-side é idempotente/indistinguível e revoga por hash, sem expor a validade do token. O logout Bearer legado `POST /auth/logout` é usado apenas quando não há refresh disponível. O Android captura o par, limpa credenciais locais e encerra RTC/JIT antes da rede; geração antiga não pode limpar ou ressuscitar uma conta nova. O estado Google também é limpo somente para a geração ainda atual. O Android nunca decide validade, rotação, revogação ou identidade: essas decisões permanecem no backend.
